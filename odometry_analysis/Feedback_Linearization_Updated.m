...
kp = 1.6;   % Proportional gain for feedback
L  = 0.04; %0.1   % Lookahead or offset distance
rate = rosrate(10);
...
for k = 0:delta:T
    ...
    % Get current pose (x_robot, y_robot, theta_robot)
    ...
    %% --- FEEDFORWARD + FEEDBACK IN CONSISTENT FRAME ---
    % Option A: Combine feedforward in body frame, then add feedback (also in body frame).
    
    % 1) Feedforward term: transform global (x_dot, y_dot) to body-frame (v, omega).
    vel_ff = [ cos(theta_robot),  sin(theta_robot);
              -sin(theta_robot)/L, cos(theta_robot)/L ] * [ x_dot(j); y_dot(j) ];
    
    % 2) Feedback term: 
    %    a) Compute position error in global frame
    ex = x(j) - x_shifted;
    ey = y(j) - y_shifted;
    
    %    b) Rotate error into the robot's local (body) frame
    R = [ cos(theta_robot),  sin(theta_robot);
    ...
    % 3) Total velocity command in body frame
    vel = vel_ff + vel_fb;
    
    % 4) Saturate the linear speed to the TurtleBot3 max
