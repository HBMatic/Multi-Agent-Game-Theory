function go_to_point_tb3_1_customodom(targetX, targetY, targetTheta)
...
msg = rosmessage(pub);
% initialize your joint_states subscriber once:
jointStateSub = rossubscriber('/tb3_1/joint_states','sensor_msgs/JointState');

% later, whenever you want fresh custom odom:
jointStateMsg = receive(jointStateSub,3.0);       % timeout = 1 s
OdomMsg = customodom(jointStateMsg);
% odomMsg now has the latest [x,y,theta] in odomMsg.Pose.Pose.Position/Orientation


%% Get Starting Pose and Generate Reference Trajectory
[startX, startY, ~] = get_current_pose_custom_odom();
numRefPoints = 100;
...
while toc < max_duration
    ...
    [x, y, theta] = get_current_pose_custom_odom();
