function [capture, capture_position, capture_type, destroyed_defenders,evade_position] = checkCaptureforinterception(Xa, Xd, Xt, par,destroyed_defenders, t, k,evade_position)
    capture = 1;
    capture_position = [];
    capture_type = '';
    ...
    if (norm(Xa - Xt) <= par.sigma(1))
        defender_distance = arrayfun(@(i) norm(Xd{i} - Xa), find(~destroyed_defenders));
        ...
        for i = find(~destroyed_defenders)
            if i == 2
                if norm(Xd{i} - Xa) <= par.sigma(3) && norm(Xd{i} - Xa) > par.sigma(1)
                    fprintf('Defender %d intercepts the Attacker at %d sec\n', i, double(t(1,k+1)));
                    capture = 0;
                ...
                if norm(Xd{i} - Xa) <= par.sigma(2) && norm(Xd{i} - Xa) > par.sigma(1)
                    fprintf('Defender %d intercepts the Attacker at %d sec\n', i, double(t(1,k+1)));
                    capture = 0;
                    capture_position = Xd{i};
                    capture_type = sprintf('defender%d', i);
