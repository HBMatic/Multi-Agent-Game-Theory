...
sub_tb3_vel = rossubscriber('/tb3_2/joint_states', 'sensor_msgs/JointState');
odomlog_tb3=[];
odomlog_gazebo=[];
figure;
...
while toc < duration    
    ...
    addpoints(h_tb3_0,x_gazebo,y_gazebo);
    addpoints(h_tb3_1,x_tb3,y_tb3);
    [vl,vr]=get_current_velocity(sub_tb3_vel);
    angular_velocity=(vr-vl)*0.2065;
    error_angular = omega-angular_velocity;
    linear_velocity=(vl+vr)*0.0165;
    error_linear= v-linear_velocity;
    drawnow;
    waitfor(rate);
    v_corrected=v_corrected+error_linear;
    omega_corrected=omega_corrected+error_angular;
    msg_tb3.Linear.X = v_corrected;
    msg_tb3.Angular.Z = omega_corrected;
end
